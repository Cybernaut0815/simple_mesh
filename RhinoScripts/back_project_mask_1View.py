...
def process_single_view(output_folder="C:\\Screenshots", view_name=None, mesh_guid=None, 
                       debug_rays=False, fov_adjustment=1.0, max_sample_points=100):
    """Process a single view from the camera data"""
    # Load camera data
    ...
    # If no view name specified, list available views and ask user to select one
    if view_name is None:
        available_views = [view["filename"] for view in camera_data["views"]]
        print("Available views:")
        for i, view in enumerate(available_views):
            print(f"{i+1}. {view}")
        
        view_index = rs.GetInteger("Select view number", 1, 1, len(available_views))
        if view_index is None:
            return None
        
        view_name = available_views[view_index-1]
    
    # Find the view info
    view_info = None
    for view in camera_data["views"]:
        if view["filename"] == view_name:
            view_info = view
            break
    
    if view_info is None:
        print(f"View {view_name} not found in camera data")
        return None
    
    # Create a layer for visualization
    base_layer = f"RayCast_{view_name.replace('.jpeg', '')}"
    ...
    # Get mask filename
    mask_filename = view_name.replace(".jpeg", "_combined_mask.png")
    mask_filepath = os.path.join(output_folder, mask_filename)
    
    # Check if mask exists
    if not os.path.exists(mask_filepath):
        print(f"Mask not found: {mask_filepath}")
        # Try looking for alternative masks with similar names
        alt_mask = view_name.replace(".jpeg", "_mask.png")
    ...
    if not os.path.exists(depth_filepath):
        print(f"Depth data not found: {depth_filepath}")
        # Try to find alternative depth data
        alt_depth = view_name.replace(".jpeg", "_depth.npy")
    ...
    if len(points_3d) == 0:
        print(f"No valid points were back-projected for view {view_name}")
        return None
    ...
    for point in points_3d:
        rs.AddPoint([point[0], point[1], point[2]])
    
    # Restore previous layer
    rs.CurrentLayer(previous_layer)
    
    print(f"Created {len(points_3d)} points for view {view_name}")
...
if __name__ == "__main__":
    ...
    # Ask for specific view to process
    camera_data_path = os.path.join(output_folder, "camera_data.json")
    if os.path.exists(camera_data_path):
        with open(camera_data_path, 'r') as json_file:
            camera_data = json.load(json_file)
        
        # Show available views for selection
        available_views = [view["filename"] for view in camera_data["views"]]
        print("Available views:")
        for i, view in enumerate(available_views):
            print(f"{i+1}. {view}")
        
        # Let user select a view by number
        view_index = rs.GetInteger("Select view number (0 to cancel)", 1, 0, len(available_views))
        ...
        print(f"Camera data not found at {camera_data_path}")
        view_name = rs.GetString("Enter view filename (e.g., view_elev30_angle0.jpeg)")
    
    # Ask for FOV adjustment factor - try 1.0 first, then adjust if needed
    ...
    # Ask if user wants to debug with visual rays
    debug_rays = rs.MessageBox("Draw debug rays to visualize sample rays?", 4) == 6  # 6 is Yes
    
    # Ask for number of sample points
    max_sample_points = rs.GetInteger("Maximum number of sample points", 100, 10, 1000)
    if max_sample_points is None:
        max_sample_points = 100  # Default
    
    # Process the selected view
    points_3d = process_single_view(
    ...
    # Ask if user wants to color the mesh
    if points_3d is not None and len(points_3d) > 0:
        if rs.MessageBox("Apply colors to mesh based on back-projected points?", 4) == 6:
            apply_colors_to_mesh(mesh_guid, points_3d)